...
@WebServlet("/Torocell")
public class Torocell extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		db=utilObj.getDb();
         ...
         {
			BasicDBObject doc = new BasicDBObject("tcc", request.getParameter("tcc"))
					.append("date", request.getParameter("Date"));
			
			
				if(request.getParameter("pp_ss") !=null)
					doc.append("pp_ss_Micromatrix", request.getParameter("pp_ss"));
					else
						doc.append("pp_ss_Micromatrix", "false");
				if(request.getParameter("pvc_pp_gl_ss_suspension") !=null)
					doc.append("pvc_pp_gl_ss_suspension", request.getParameter("pvc_pp_gl_ss_suspension"));
				else
					doc.append("pvc_pp_gl_ss_suspension", "false");
			
				doc.append("torocellSystemIdNo",request.getParameter("torocell_system_id_no"))
				.append("barcode", request.getParameter("barcode").toString().trim());
				...
				doc.append("Remarks_OnTheSystem", request.getParameter("Remarks").toString().trim());
				...
				for(int i=0;i<cmnoArray.length;i++)
				{
					...
					if(!cmnoArray[i].equals("") || !noOFContainerArray[i].equals("") || !yieldArray[i].equals("") || !cellyieldArray[i].equals("") || !totalcellArray[i].equals(""))
					{
				...
				.append("settingUpCulture",request.getParameter("settingUpCulture").toString().trim());
				...
				doc.append("TerminationCulture", request.getParameter("termination_culture").toString().trim())
         ...
         response.sendRedirect("Torocell.jsp?message=" + URLEncoder.encode(message, "UTF-8"));
         }
         
         else
         {
        	 response.sendRedirect("Torocell.jsp");	 
         }
	}
